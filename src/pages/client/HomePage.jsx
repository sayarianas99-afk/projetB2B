...
useEffect(() => {
  ...
  return (
    <div className="animate-fade-in">
      {/* Hero */}
      <section className="hero-gradient hero-mesh min-h-[520px] flex items-center relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16 relative z-10 w-full">
          <div className="max-w-2xl">
            ...
            <form onSubmit={handleSearch} className="flex gap-3 max-w-xl">
              ...
              <button type="submit" className="btn-accent py-3.5 px-6 rounded-2xl text-sm font-bold">{t('searchBtn')}</button>
            </form>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {categories.map(cat => (
              <Link key={cat.id} to={`/products?category=${cat.id}`}
                className="card-hover p-4 flex flex-col items-center gap-2 text-center group dark:bg-dark-800 dark:border-white/5">
                <span className="text-3xl">{cat.icon}</span>
                <span className="text-xs font-semibold text-gray-700 group-hover:text-primary-600 transition-colors dark:text-gray-300 dark:group-hover:text-primary-400">{cat.name}</span>
              </Link>
            ))}
          </div>
        </section>
